/***********************************************************************
  ...
  ============================================================
  Writes ONLY page 0 (0x0000-0x03FF, the bootloader) to flash via the
  debug interface (DC/DD/RESET), using the "CPU Flash Write" method
  from the TI SWRU283B User's Guide (6.2.1/6.2.4) - not the DMA method
  (the datasheet's "preferred method", faster, but damaged on this
  project's dev dongle, chip ID 4414: isolated by comparing both
  methods, DMA always failed even for a single byte despite a verified
  correct erase and clean chip ID/status).

  Refuses to write any byte at 0x0400 or beyond (app area) - use
  write_app_via_debug for the app area, which has the opposite
  restriction (never page 0). Doesn't erase the page itself - erase
  first with erase_bootloader_page_via_debug; the write only checks it
  is already 0xFF where needed.
  ...
  never ported/tested here: if a future healthy chip needs faster
  writes, write an equivalent DMA-based tool following
  `ram_flash_write_dma.c` (never used via debug, only compiled).

  Typical use (bootstrapping a new dongle, first time only - afterward
  use USB only, see the parent folder's README.md):
    ./erase_chip_via_debug              # erase the whole chip
    ./write_bootloader_via_debug ../bootloader/bin/bootloader.ihx
...
void helpo()
{
  fprintf(stderr,"usage : write_bootloader_via_debug [-d pin_DD] [-c pin_DC] [-r pin_reset] file_to_flash\n");
  fprintf(stderr,"  Refuses any byte at 0x0400 or beyond (app area).\n");
...
int main(int argc,char *argv[])
{
  ...
  if (maxpage > 0 || Pages[0].maxoffset >= 0x0400) {
    fprintf(stderr, "SAFETY ERROR: the file contains bytes at 0x0400 or beyond "
                     "(app area) - this tool writes ONLY page 0 (the bootloader). "
                     "Use write_app_via_debug for the app area.\n");
    exit(1);
  }
